...
def add_water_mark():
    ...
    for image_path in os.listdir("images"):
        if not image_path.endswith("png"):
            continue
        filename = os.path.join("docs/images", image_path)
        if image_path not in marked:
            ...
            rotated_font = ImageFont.truetype(
                "font.ttf", font_size
            )  # 使用同样大小的字体，以保持字号不变
            rotated_text_width, rotated_text_height = draw.textsize(
                rotated_text, rotated_font
            )
            rotated_text_image = Image.new(
                "RGBA", (rotated_text_width, rotated_text_height), (0, 0, 0, 0)
            )
            rotated_text_draw = ImageDraw.Draw(rotated_text_image)
            rotated_text_draw.text(
                (0, 0), rotated_text, fill=(0, 0, 255, 180), font=rotated_font
            )
            rotated_text_image = rotated_text_image.rotate(
                angle, expand=True
            )  # 旋转文本图像
            watermark_image.paste(
                rotated_text_image, (x, y), rotated_text_image
            )  # 将旋转后的文本粘贴到水印图像上

            # 将水印图像叠加到原始图片上
            watermarked_image = Image.alpha_composite(
                original_image.convert("RGBA"), watermark_image
            )
