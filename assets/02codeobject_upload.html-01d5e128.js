import{_ as p}from"./qrcode2-187b7271.js";import{_ as c,r as l,o,c as i,a as n,d as s,b as e,e as t}from"./app-6b5e6c63.js";const r={},u=n("h1",{id:"字节码灵魂——code-obejct",tabindex:"-1"},[n("a",{class:"header-anchor",href:"#字节码灵魂——code-obejct","aria-hidden":"true"},"#"),s(" 字节码灵魂——Code obejct")],-1),d={href:"https://mp.weixin.qq.com/s?__biz=Mzg3ODgyNDgwNg==&mid=2247488040&idx=1&sn=665b5b6080d5ec7910f586b252281bcf&chksm=cf0c8e21f87b073748c82af61a5c7c9d73bca95e5b6558d50d1d5b1cc97e50c4a93d9daffcfa&token=1257007364&lang=zh_CN#rd",target:"_blank",rel:"noopener noreferrer"},m=t(`<h2 id="code-object-数据结构" tabindex="-1"><a class="header-anchor" href="#code-object-数据结构" aria-hidden="true">#</a> Code Object 数据结构</h2><div class="language-c line-numbers-mode" data-ext="c"><pre class="language-c"><code><span class="token keyword">typedef</span> <span class="token keyword">struct</span> <span class="token punctuation">{</span>
    PyObject_HEAD
...
</code></pre><div class="line-numbers" aria-hidden="true"><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div></div></div>`,7),b={href:"https://github.com/Chang-LeHung/dive-into-cpython/blob/master/code/codeobject/co01.py",target:"_blank",rel:"noopener noreferrer"},v=t(`<div class="language-bash line-numbers-mode" data-ext="sh"><pre class="language-bash"><code>code
...
<span class="token macro property"><span class="token directive-hash">#</span><span class="token directive keyword">define</span> <span class="token macro-name">CO_ITERABLE_COROUTINE</span>   <span class="token expression"><span class="token number">0x0100</span></span></span>
...
</code></pre><div class="line-numbers" aria-hidden="true"><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div></div></div>`,12),k={href:"https://github.com/Chang-LeHung/dive-into-cpython/blob/master/code/codeobject/co01.py",target:"_blank",rel:"noopener noreferrer"},_=t(`<div class="language-bash line-numbers-mode" data-ext="sh"><pre class="language-bash"><code>code
 ...
 <span class="token number">26</span>          <span class="token number">12</span> LOAD_FAST                <span class="token number">0</span> <span class="token punctuation">(</span>a<span class="token punctuation">)</span>
   ...
   freevars <span class="token punctuation">(</span><span class="token punctuation">)</span>
   cellvars <span class="token punctuation">(</span><span class="token punctuation">)</span>
</code></pre><div class="line-numbers" aria-hidden="true"><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div></div></div><p>我们现在来模拟一下执行过程，在模拟之前我们首先来了解一下上面几条字节码的作用：</p><ul><li>LOAD_CONST，将常量表当中的下标等于 i 个对象加载到栈当中，对应上面的代码 LOAD_CONST 的参数 i = 1。因此加载测常量等于 1 。因此现在栈空间如下所示：</li></ul><p><img src="https://img2023.cnblogs.com/blog/2519003/202304/2519003-20230402013032077-1272789285.png" alt=""></p><ul><li>STORE_FAST，将栈顶元素弹出并且保存到 co_varnames 对应的下标当中，根据上面的字节码参数等于 0 ，因此将 1 保存到 co_varnames[0] 对应的对象当中。</li></ul><p><img src="https://img2023.cnblogs.com/blog/2519003/202304/2519003-20230402013032778-622122724.png" alt=""></p><ul><li>LOAD_CONST，将下标等于 2 的常量加载进入栈中。</li></ul><p><img src="https://img2023.cnblogs.com/blog/2519003/202304/2519003-20230402013033122-1213368211.png" alt=""></p><ul><li>STORE_FAST，将栈顶元素弹出，并且保存到 varnames 下标为 1 的对象。</li></ul><p><img src="https://img2023.cnblogs.com/blog/2519003/202304/2519003-20230402013033479-1762782583.png" alt=""></p><ul><li>LOAD_FAST，是取出 co_varnames 对应下标的数据，并且将其压入栈中。我们直接连续执行两个 LOAD_FAST 之后栈空间的布局如下：</li></ul><p><img src="https://img2023.cnblogs.com/blog/2519003/202304/2519003-20230402013033956-1755763135.png" alt=""></p><ul><li>BINARY_ADD，这个字节码指令是将栈空间的两个栈顶元素弹出，然后将两个数据进行相加操作，然后将相加得到的结果重新压入栈中。</li></ul><p><img src="https://img2023.cnblogs.com/blog/2519003/202304/2519003-20230402013034310-1459420418.png" alt=""></p><ul><li>RETURN_VALUE，将栈顶元素弹出并且作为返回值返回。</li></ul><p>从上面的整个执行过程来看整个栈空间使用的最大的空间长度为 2 ，因此 stacksize = 2 。</p><h2 id="总结" tabindex="-1"><a class="header-anchor" href="#总结" aria-hidden="true">#</a> 总结</h2><p>在本篇文章当中主要分析了一些 code obejct 当中比较重要的字段，code object 是 cpython 虚拟机当中一个比较重要的数据结构，深入的去理解这里面的字段对于我们理解 python 虚拟机非常有帮助。</p><hr><p>本篇文章是深入理解 python 虚拟机系列文章之一，文章地址：https://github.com/Chang-LeHung/dive-into-cpython</p>`,27),g={href:"https://github.com/Chang-LeHung/CSCore",target:"_blank",rel:"noopener noreferrer"},h=n("p",null,"关注公众号：一无是处的研究僧，了解更多计算机（Java、Python、计算机系统基础、算法与数据结构）知识。",-1),f=n("p",null,[n("img",{src:p,alt:""})],-1);function O(y,A){const a=l("ExternalLinkIcon");return o(),i("div",null,[u,n("p",null,[s("在本篇文章当中主要给大家深入介绍在 cpython 当中非常重要的一个数据结构 code object! 在上一篇文章 "),n("a",d,[s("深入理解 python 虚拟机：pyc 文件结构"),e(a)]),s(" ，我们简单介绍了一下在 code object 当中有哪些字段以及这些字段的简单含义，在本篇文章当中将会举一些例子以便更加深入理解这些字段。")]),m,n("p",null,[s("在前面的文章当中我们提到过一个函数是包括一个 code object 对象，test_co01 的 code object 对象的输出结果（完整代码见"),n("a",b,[s("co01"),e(a)]),s("）如下所示：")]),v,n("p",null,[s("上面的函数的信息如下所示（完整代码见"),n("a",k,[s("co02"),e(a)]),s("）：")]),_,n("p",null,[s("更多精彩内容合集可访问项目："),n("a",g,[s("https://github.com/Chang-LeHung/CSCore"),e(a)])]),h,f])}const T=c(r,[["render",O],["__file","02codeobject_upload.html.vue"]]);export{T as default};
